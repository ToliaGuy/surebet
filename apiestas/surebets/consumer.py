...
from capiestas.api.app.models.matches import MatchInDB
...
def run_consumer(group_id : str, broker_url: str, registry_url: str, subscription_name: str, apiestas_url: str):
    ...
    while True:
        ...
        # checks if there are no surebets than it proceeds to calculate surebets
        # if there are surebets it checks recency of every if if they were created less than one minute ago
        # it doesnt calculate surebets
        if not (match.surebets
                and datetime.utcnow() - min(surebet.created_at for surebet in match.surebets) < timedelta(minutes=1)):
            # not sure how does this script deletes old surebets, that are no more relevant
